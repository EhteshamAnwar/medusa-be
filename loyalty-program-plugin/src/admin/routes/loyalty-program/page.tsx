...
const LoyaltyProgram = () => {
  const { data, isLoading } = useAdminCustomQuery<RequestQuery, ResponseData>(
    ...
    const transformResponseData = (data: Record<string, any>): any[] => {
        const dataKeys = Object.keys(data).filter(key => !isNaN(Number(key)));
        return dataKeys.map(key => data[key as unknown as number]);
    };
